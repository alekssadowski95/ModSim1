...
params.obstacle_p      = 0.01;

[G, A, O] = groundwater_sim(depth, width, params);

spy(G);
%spy(A);
%spy(O);

function [ground, absorb_mat, obstacle_mat] = groundwater_sim(depth, width, params)
...
%   - params.obstacle_p:         Probability for solid obstacles to appear in the obstacle matrix
%
% Outputs:
...
%                   It is a (depth x width) Matrix with values in {0,1}
%   - obstacle_mat: 2D-grid representing  the solid obstacle material 
%                   It is a (depth x width) Matrix with values in {0,1}
...
% create absorption matrix
obstacle_mat = zeros(81, 250);
for i = 1:81
    for j = 1:250
        obstacle_mat(i, j) = rand <= params.obstacle_p;  
    end
end

%obstacle_mat(1:5,1:10)

% Run a simulation
ground = compute_ground(ground_init, absorb_mat, obstacle_mat, params);
end


function ground = compute_ground(ground, absorb_mat, obstacle_mat, params)
...
for i = 1:ground_depth
    ...
    for j = 1:ground_width
        ...
        if ground(i, j) == 1
            ...
            if ground(i, j) == 1
                % checks whether there is an ostacle in the lower cell
                if obstacle_mat(i+1, j) == 1
                    % checks whether the water seeps through to the left lower ground cell 
                    % the depth influences the probability to seep through.
                    if j == 1
                        if i < ground_depth
                            ground(i+1, j+1) = rand <= (params.probability*(1-params.depth_influence)^i);
                        end
